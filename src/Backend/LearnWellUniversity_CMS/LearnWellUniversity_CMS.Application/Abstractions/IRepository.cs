...
public interface IRepository<T> where T : class
{
    ...
    IQueryable<T> GetByFiltersAsQueryable(Expression<Func<T, bool>>? filter = null);
    Task<List<TResponseType>> GetByFiltersAsync<TResponseType>(Expression<Func<T, bool>>? filter = null, CancellationToken cancellationToken = default) where TResponseType : class;
    Task<PaginatedResponse<TResponseType>> GetByFiltersAsync<TResponseType>(Expression<Func<T, bool>>? filter = null, int? page = 0, int? pageSize = null, CancellationToken cancellationToken = default) where TResponseType : class;
